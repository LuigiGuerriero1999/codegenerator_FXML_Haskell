...
main = withScopedPtr (getArgs >>= QApplication.new) $ \_ -> do
  ...
  -- linkerkant aanmaken
  label1 <- QLabel.newWithText "Typ hier uw tekst:"   -- eerste label (bovenaan) 
  textEdit <- QTextEdit.new                           -- textedit (in het midden) 
  QWidget.setFixedSizeRaw textEdit 150 30
  label2 <- QLabel.newWithText "Hier komt de tekst"   -- tweede label (onderaan) 
  linkerkant <- QWidget.new
  QWidget.setFixedSizeRaw linkerkant 300 300
  linkerkantLayout <- QVBoxLayout.new        -- een Layout (anchorpane, links)
  ...
  rechterkantLayout <- QVBoxLayout.new             -- een Layout (anchorpane, rechts)
  QWidget.setLayout rechterkant rechterkantLayout
  QBoxLayout.addWidget rechterkantLayout button    -- button meegeven layout rechts

  -- splitter aanmaken die  widget (window) in twee delen verdeeld 
  splitter <- QSplitter.new
  QSplitter.addWidget splitter linkerkant
  QSplitter.addWidget splitter rechterkant
  
  -- globale layout (anchorpane) met parent de window en waar de splitter wordt meegegeven
  layout <- QVBoxLayout.newWithParent window  
  QBoxLayout.addWidget layout splitter

  -- ui definiëren met de window, tweede label 
  --(omdat de tekst hiervan moet veranderen) en de tekst van de textedit
  let ui = UI { uiWindow = window
              , uiLabel2 = label2
              , uiTextEdit = QTextEdit.toPlainText textEdit}

  -- een signaal koppelen aan de button om een actie (toontekst) uit te voeren
  connect_ button QAbstractButton.clickedSignal $ \_ -> toonTekst ui  
